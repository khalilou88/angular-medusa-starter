      ...
      <div class="absolute right-0 top-0 h-full w-full max-w-md bg-white shadow-xl transform transition-transform"
        ...
        <div class="flex-1 overflow-y-auto p-4">
          <div *ngIf="cart?.items && cart.items.length > 0; else emptyCart">
            <div *ngFor="let item of cart.items; trackBy: trackByItemId" 
                 class="flex items-center space-x-4 py-4 border-b border-gray-200 last:border-b-0">
              
              <!-- Item Image -->
              <div class="flex-shrink-0">
                <img [src]="item.thumbnail || '/assets/placeholder-product.jpg'" 
                     [alt]="item.title"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p *ngIf="item.description" class="text-sm text-gray-500 truncate">{{ item.description }}</p>
          ...
          <ng-template #emptyCart>
            <div class="text-center py-12">
              <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                      d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-1.5 6M7 13l-1.5 6m0 0h9M7 13h10"></path>
              </svg>
              <h3 class="mt-2 text-sm font-medium text-gray-900">Your cart is empty</h3>
              <p class="mt-1 text-sm text-gray-500">Add some items to get started.</p>
              <button (click)="closeAndNavigate('/products')" 
        ...
        <div *ngIf="cart?.items && cart.items.length > 0" class="border-t p-4 space-y-4">
          ...
          <div class="space-y-2">
            <div class="flex justify-between text-sm">
              ...
              <span class="font-medium">{{ (cart.subtotal / 100) | currency:'USD':'symbol':'1.2-2' }}</span>
            </div>
            <div *ngIf="cart.shipping_total > 0" class="flex justify-between text-sm">
              <span class="text-gray-600">Shipping</span>
              <span class="font-medium">{{ (cart.shipping_total / 100) | currency:'USD':'symbol':'1.2-2' }}</span>
            </div>
            <div *ngIf="cart.tax_total > 0" class="flex justify-between text-sm">
              <span class="text-gray-600">Tax</span>
              <span class="font-medium">{{ (cart.tax_total / 100) | currency:'USD':'symbol':'1.2-2' }}</span>
            </div>
